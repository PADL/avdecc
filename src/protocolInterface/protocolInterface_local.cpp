...
public:
	...
	ProtocolInterfaceLocalImpl(std::string const& networkInterfaceName, std::string const& executorName)
		: ProtocolInterfaceLocal(networkInterfaceName, executorName)
	{
		// open socket
		sockaddr_un sun{};
		...
		if (err < 0)
		{
			throw Exception(Error::TransportError, "Faild to connect local domain socket to peer");
		}
...
ProtocolInterfaceLocal::ProtocolInterfaceLocal(std::string const& networkInterfaceName, std::string const& executorName)
